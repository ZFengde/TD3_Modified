...
class Networks(nn.Module): # This network is for generating x_t-1 or epsilon_theta
    ...
    def forward(self, action, time, state):
        ...
        if action.dim()==3:
            t_embed = self.time_mlp(time).unsqueeze(1).expand(-1, n_actions, -1) # time embedding
        else:
...
class Diffusion_Policy(nn.Module): # forward method here is generate a sample
    ...
    def p_losses(self, x_start, state_feat, t): # TODO, expand the noise to have multiple choice
        noise = th.randn_like(x_start)
    ...
    def loss(self, x, state_feat): # TODO, expand the noise to have multiple choice
